...
beta.behave<-0
beta.sex<-0
# start beta1=0 so if there is no covariate this parameter is zeroed out
beta1<-0
...
start.time <- Sys.time()
	...
	for(i in 1:ni){
	  print((sprintf("This is iteration number %i of chain %i \n with sex value as %i", i,ch,Msex)))
########################
########################
########################
########################
...
lp<-   loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + Msex*beta.sex*Xsex[indid]
repeat{
## theta must not be less than 0.5 or greater than 1 ## 
