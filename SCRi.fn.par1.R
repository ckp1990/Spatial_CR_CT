setwd("/home/ravi/Desktop/SCR_Analysis/SCRbayes/TGR/BD2014")
library(SCRbayes)
library(doMC)
captures.tigers <- read.csv("Capture1.csv")
traps.tigers <- read.csv("Trap.csv")
statespace.tigers <- read.csv("Mask.csv")
scrobj <- scrData(traps.tigers,captures.tigers,statespace.tigers)
XSex <- read.csv(file="Xsex_F1.csv", header=TRUE)
XSex <- XSex[,2]

SCRi.fn.par1 <-function(scrobj,modelno=1, nc=3,ni=5000,burn=100,skip=2,nz=400,theta=NA,Msigma=1,Mb=0,Msex=1,
                        Msexsigma = 1,Xeff=NULL,Xsex=XSex, ss.prob=NULL,coord.scale=1000,area.per.pixel=0.3139,
...
## Alternate e2dist function from SPACECAP
	e2dist <- function(A, B)  {
		xdif <- outer(A[, 1], B[, 1], "-")
		ydif <- outer(A[, 2], B[, 2], "-")
		sqrt(xdif^2 + ydif^2)
...
## Create a folder ##
folderName <- paste("MLDresultsModel_",modelno,"_", ts,"CH",ch,sep="")
...
NW14_1 <- SCRi.fn.par1(scrobj,modelno=1, nc=3,ni=10000,burn=10,skip=2,nz=400,theta=1,Msigma=1,Mb=0,Msex=1,
                       Msexsigma =1,Xeff=NULL,Xsex=XSex, ss.prob=NULL,coord.scale=1000,area.per.pixel=0.3139,
                       thinstatespace=1,maxNN=20,dumprate=1000)
